...
import pl.supra.readstack.domain.category.Category;
import pl.supra.readstack.domain.category.CategoryDao;
...
public class CategoryService {
    ...
    private static class CategoryNameMapper {
        static CategoryName map(Category c){
            return new CategoryName(c.getId(), c.getName());
        }
    }
    ...
    private static class CategoryFullInfoMapper {
        static CategoryFullInfo map(Category c){
            return new CategoryFullInfo(c.getId(), c.getName(), c.getDescription());
        }
    }
}
